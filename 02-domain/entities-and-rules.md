...
> Autor: ADSO 3145555 | Equipo: Arquitectura / Dominio

## Contexto

Describe las entidades principales del dominio y las reglas de negocio que rigen su comportamiento, agrupadas por módulo funcional.

## Regla de oro

> Ninguna asignación de horario es válida si el instructor, el ambiente o la ficha están inactivos, en conflicto o fuera de su ámbito de competencia.
...
| **Centro de Formación** | Unidad principal encargada de ejecutar procesos formativos | `id`, `microregion_id`, `codigo`, `nombre`, `tipo`, `estado` |
...
| **Disponibilidad** | Estado actual del ambiente | `id`, `ambiente_id`, `estado`, `fecha_actualizacion` |

### Reglas de negocio

- **RN-005:** Un ambiente solo puede estar en un estado a la vez: Disponible, Ocupado, Mantenimiento o Fuera de servicio.
...
| **Estado** | Estados transversales del sistema (Activo, Inactivo, Suspendido, Finalizado) | `id`, `codigo`, `nombre`, `descripción`, `color`, `activo` |
...
- **RN-011:** Los parámetros con `editable=false` solo pueden ser modificados por administradores.
- **RN-012:** Todo cambio en catálogos o parámetros queda registrado en auditoría.
...
- **RN-013:** Un programa de formación Titulado no puede ser `a_la_medida` — ese atributo solo aplica para Complementaria.
...
- **RN-017:** La jerarquía Línea → Red Tecnológica → Red de Conocimiento → Programa debe respetarse sin excepción.
...
| Entidad | Descripción | Atributos clave |
|---------|-------------|-----------------|
| **Instructor** | Persona que orienta la formación. Puede ser planta o contratista. | `id`, `nombre`, `email`, `tipo`, `especialidades`, `estado` |
| **Aprendiz** | Persona que recibe formación, matriculada en una ficha. | `id`, `nombre`, `documento`, `email`, `ficha_id`, `estado` |
...
- **RN-021:** Solo los instructores habilitados en una competencia pueden ser asignados a bloques de esa competencia.

---

## Módulos 8 y 11 — Horarios y Motor de Asignación
...
| **Horario** | Asignación de instructor + ambiente + ficha + franja horaria | `id`, `instructor_id`, `ambiente_id`, `ficha_id`, `franja_id`, `fecha`, `estado` |
...
| **Conflicto** | Cruce detectado de instructor, ambiente o ficha | `id`, `horario_id`, `tipo_conflicto`, `descripción`, `fecha_detección` |
| **Sesión de Clase** | Ejecución real de una sesión de formación | `id`, `horario_id`, `fecha_ejecucion`, `hora_inicio_real`, `hora_fin_real`, `estado` |
...
- **RN-027:** Una sesión de clase ya ejecutada no puede modificarse — solo se puede añadir una observación.
...
| **Entregable** | Producto o resultado de una actividad | `id`, `actividad_id`, `nombre`, `descripción`, `url_evidencia`, `estado` |
| **Evidencia** | Soporte digital de un entregable | `id`, `entregable_id`, `tipo`, `url`, `fecha_subida` |
| **Dependencia** | Relación de orden entre fases | `id`, `fase_origen_id`, `fase_destino_id`, `tipo` |
...
- **RN-030:** El avance del proyecto se calcula como porcentaje de fases y actividades completadas sobre el total.
...
| Entidad | Descripción | Atributos clave |
|---------|-------------|-----------------|
| **Usuario** | Persona con acceso al sistema | `id`, `nombre`, `email`, `password_hash`, `activo` |
...
- **RN-034:** Los tokens JWT expiran a las 8 horas y deben renovarse.
- **RN-035:** Todas las acciones críticas (crear, modificar, eliminar) se registran en auditoría.

---

## Resumen de reglas críticas — Motor de horarios

| ID | Regla | Afecta a |
|----|-------|----------|
| RN-022 | Un instructor no puede estar en dos horarios solapados | Horarios |
| RN-023 | Un ambiente no puede estar en dos horarios solapados | Horarios |
| RN-024 | Una ficha no puede tener dos horarios solapados | Horarios |
| RN-025 | No se asignan horarios a entidades inactivas | Horarios |
| RN-027 | Una sesión ejecutada no puede modificarse | Ejecución |
| RN-006 | Un ambiente en mantenimiento no puede asignarse | Ambientes |
| RN-019 | Un instructor inactivo no puede asignarse | Actores |
| RN-016 | La competencia debe pertenecer al programa de la ficha | Académico |
| RN-013 | Un programa Titulado no puede ser `a_la_medida` | Programas |
| RN-014 | La habilitación del instructor es por competencia, no por RAP | Programas |
